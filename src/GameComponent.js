    ...
    if(ctx) {
      ...
      for (let i = 0; i < this.state.enemies.length; i++) {
          ...
          if (this.state.enemies[i].positionX > -50) {
            ...
            // this.state.enemies[i].positionX -= this.state.enemies[i].speed;
          } else {
            let newEnemies = this.state.enemies;
            ...
            // this.state.enemies[i] = this.resetEnemy(this.state.enemies[i]);
          }
      }
    }
};
...
resetEnemy(invader) {
    ...
    invader.radius = this.randomNumber(this.state.gameSettings.minInvaderSize, this.state.gameSettings.maxInvaderSize);
